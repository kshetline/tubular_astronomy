...
export class SaturnMoons extends PlanetaryMoons {
  ...
  protected getMoonPositionsAux(time_JDE: number, sunPerspective: boolean): MoonInfo[] {
    ...
    let λ = 0.0, r = 0.0, γ = 0.0, Ω = 0.0, K = 0.0;
    let W: number;
    let L: number;
    let p = 0.0;
    let M: number;
          ...
          switch (j) {
            ...
            case 6: // VII, Hyperion
              ...
              e = 0.103458 - 0.004099 * cos_deg(η) - 0.000167 * cos_deg(ζ + η)
                  ...
                  - 0.00212 * cos_deg(2.0 * ζ)
                  + 0.000151 * cos_deg(3.0 * ζ) + 0.00013 * cos_deg(φ);
              p = ww + 0.15648 * sin_deg(χ) - 0.4457 * sin_deg(η) - 0.2657 * sin_deg(ζ + η)
                  - 0.3573 * sin_deg(ζ - η) - 12.872 * sin_deg(ζ) + 1.668 * sin_deg(2.0 * ζ)
                  - 0.2419 * sin_deg(3.0 * ζ) - 0.07 * sin_deg(φ);
              λ1 = 177.047 + 16.91993829 * t6 + 0.15648 * sin_deg(χ) + 9.142 * sin_deg(η)
                      + 0.007 * sin_deg(2.0 * η) - 0.014 * sin_deg(3.0 * η)
                      + 0.2275 * sin_deg(ζ + η)
            ...
            case 7: // VII, Iapetus
              ...
              i1 = 18.4602 - 0.9518 * t11 - 0.072 * t11 ** 2 + 0.0054 * t11 ** 2 * t11;
              Ω1 = 143.198 - 3.919 * t11 + 0.116 * t11 ** 2 + 0.008 * t11 ** 2 * t11;
              l = μ - ww0;
              g = ww0 - Ω1 - ψ;
              g1 = ww0 - Ω1 - φ;
              ls = W5 - ww1;
              ...
              ω = 0.08077 * sin_deg(g1 - gT) + 0.02139 * sin_deg(u5 - 2.0 * g) - 0.00676 * sin_deg(u3)
                  + 0.01380 * sin_deg(l) + 0.01632 * sin_deg(l + u2) + 0.03547 * sin_deg(u4);
              p = ww0 + ω / e1;
              λ1 = μ - 0.04299 * sin_deg(u2) - 0.00789 * sin_deg(u1) - 0.06312 * sin_deg(ls)
                      - 0.00295 * sin_deg(2.0 * ls) - 0.02231 * sin_deg(u5) + 0.00650 * sin_deg(u5 + ψ);
